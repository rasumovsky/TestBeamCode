...
plotTH2F(TH2F *h2, TString xname, TString yname, TString zname, TString sname, 
	 double x1=0, double x2=0, double y1=0, double y2=0, double z1=0,
	 double z2=0) {
  ...
  h->GetYaxis()->SetTitle(yname);
  if (y1 != 0 || y2 != 0) h->GetYaxis()->SetRangeUser(y1,y2);
  h->GetZaxis()->SetTitle(zname);
  if (z1 != 0 || z2 != 0) h->GetZaxis()->SetRangeUser(z1,z2);
  can->Print(Form("%s/%s.eps",outputDirectory.Data(),sname.Data()));
  can->Print(Form("%s/%s.pdf",outputDirectory.Data(),sname.Data()));
  can->Clear();
}
